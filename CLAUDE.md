...
- **Meta tag e Open Graph** in `app/layout.tsx`: `metadataBase` (necessario perché Next risolva gli URL assoluti di OG/Twitter), `openGraph` e `twitter` completi, immagine social = `public/images/logo.png` (512×512, quadrata — non è una vera social card 1200×630, va sostituita quando si avrà un'immagine dedicata).
...
Nessun i18n IT/ES implementato (fase 2). Con Blocchi 1-7 completi, i rimanenti passi sono quelli di `design/NEXT-STEPS.md` §5-6: deploy su Vercel, DNS, e la checklist pre-lancio (case study reali, conferma numeri pubblici, test form con chiave reale, test mobile su device fisico, test da tastiera end-to-end).
